...
export default function TestersPage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="bg-surface-alt border-b-[0.5px] border-bronze/20 py-20">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <p className="section-subheading mb-4">Discovery Collection</p>
          <h1 className="font-cormorant text-5xl md:text-6xl lg:text-7xl font-light text-theme-primary leading-[1.05] mb-6">
          ...
          <p className="font-inter text-sm md:text-base text-theme-secondary leading-relaxed max-w-2xl mx-auto">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <p className="font-inter text-[10px] tracking-[0.25em] uppercase text-theme-secondary mb-2">All 5 Fragrances Available as Testers</p>
        <h2 className="font-cormorant text-3xl font-light text-theme-primary mb-10">The Full Collection — 10ml</h2>
              ...
              >
                ...
                <div className="flex justify-center md:justify-start">
                  <Link href={`/products/${product.slug}`}>
                    <div className="relative w-28 h-36 bg-surface-alt overflow-hidden flex-shrink-0 hover:opacity-90 transition-opacity">
                      <Image
                ...
                <div>
                  ...
                  <Link href={`/products/${product.slug}`}>
                    <h3 className="font-cormorant text-3xl font-light text-theme-primary hover:text-bronze transition-colors mb-1">
                      {product.name}
                    </h3>
                  </Link>
                  <p className="font-inter text-sm text-theme-secondary italic mb-3">{product.tagline}</p>
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {[...product.notes.top, ...product.notes.heart.slice(0, 2)].map(note => (
                      <span
                        key={note}
                        className="font-inter text-[10px] text-theme-primary bg-theme border-[0.5px] border-bronze/25 px-2.5 py-0.5"
                      >
                        {note}
                      </span>
                    ))}
                  </div>
                  <p className="font-inter text-xs text-theme-secondary max-w-lg leading-relaxed">
                ...
                <div className="text-left md:text-right flex-shrink-0">
                  <p className="font-inter text-[10px] tracking-[0.15em] uppercase text-theme-secondary mb-1">10ml Tester</p>
                  <p className="font-cormorant text-4xl font-light text-theme-primary mb-1">
                    ₹{testerSize.price.toLocaleString('en-IN')}
                  </p>
                  {fullSize && (
                    <p className="font-inter text-[11px] text-theme-secondary mb-4">
      ...
      <div className="bg-surface-alt border-t-[0.5px] border-b-[0.5px] border-bronze/20 py-16">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <p className="section-subheading mb-3">Try All Five</p>
          <h2 className="font-cormorant text-4xl md:text-5xl font-light text-theme-primary mb-4">
            The Discovery Set
          </h2>
          <div className="w-12 h-[0.5px] bg-bronze mx-auto mb-6" />
          <p className="font-inter text-sm text-theme-secondary leading-relaxed max-w-xl mx-auto mb-4">
          ...
          <p className="font-inter text-xs text-theme-secondary mb-8 italic">
          ...
          <div className="flex justify-center gap-3 mb-10 flex-wrap">
            {products.map(p => {
              ...
              <Link key={p.id} href={`/products/${p.slug}`}>
                <div className="relative w-16 h-20 bg-theme overflow-hidden hover:opacity-80 transition-opacity">
                  <Image
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            <div key={item.headline} className="border-t-[0.5px] border-bronze/30 pt-5">
              <p className="font-inter text-xs font-medium tracking-[0.1em] uppercase text-theme-primary mb-2">{item.headline}</p>
              <p className="font-inter text-xs text-theme-secondary leading-relaxed">{item.body}</p>
            </div>
          ))}
        </div>
      </div>
